...
class ViagemOnibusApp:
    def __init__(self, root):
        ...
        # Rótulo para mostrar a média de consumo
        self.label_media_combustivel = tk.Label(self.root, text="Média de Combustíveis:")
        self.label_media_combustivel.pack()
    ...
    def salvar_csv(self):
        try:
            ...
            # Calcular e exibir a média de consumo
            self.calcular_media_consumo(arquivo_csv)
    ...
    def calcular_media_consumo(self, arquivo_csv):
        try:
            # Ler o arquivo CSV e calcular a média de litros abastecidos
            litros_totais = 0
            viagens_totais = 0
            ...
            with open(arquivo_csv, "r") as file:
                reader = csv.DictReader(file)
                for linha in reader:
                    litros_totais += float(linha["Litros Abastecidos"])
                    viagens_totais += 1

            if viagens_totais > 0:
                media_consumo = litros_totais / viagens_totais
                self.label_media_combustivel.config(text=f"Média de Combustíveis: {media_consumo:.2f} litros")
            else:
                self.label_media_combustivel.config(text="Nenhuma viagem registrada para calcular a média")
        except Exception as e:
            messagebox.showerror("Erro", f"Ocorreu um erro ao calcular a média de consumo: {e}")
